...
import omrsynth.util.blendops
import omrsynth.util.general
import omrsynth.util.noises
...
def _blurred_noise(shape, kernel_size, y_min=0.0, y_max=1.0):
    ...
    noise = omrsynth.util.general.linear_transform(noise, y_min=y_min, y_max=y_max)

    # Copy grayscale to three channels:
    return omrsynth.util.general.repeat_to_3_channels(noise)
...
def _random_add(scale=1.0):
    ...
    :return: Uniformly distributed random value in the range of
    scale * [-1.0; 1.0]
    """
...
def _high_freq_hsv_noise(shape, shift_x=0.0, shift_y=0.0, rnd_scale=0.0):
    ...
    mask = omrsynth.util.noises.simplex_noise(shape=shape, octaves=2,
    ...
    noise = omrsynth.util.general.linear_transform(noise, x_min=0, x_max=1,
                                                   y_max=y_max_randomized)
    noise = noise + salt
    omrsynth.util.general.clip_image(noise)
    ...
    salt = skimage.util.img_as_float(np.random.rand(shape[0],
                                                    shape[1]) > salt_amount)
    mask += salt
    omrsynth.util.general.clip_image(mask)
    ...
    value = omrsynth.util.general.linear_transform(value, y_min=0, y_max=1, x_min=x_min_randomized, x_max=1)
    ...
    value = omrsynth.util.general.linear_transform(value, y_min=0, y_max=0.1)

    # Bright spots
    bs = np.random.rand(shape[0], shape[1])
    bs = omrsynth.util.general.linear_transform(bs, y_min=0, y_max=1, x_min=0.995, x_max=1)
    kernel_blur = np.matrix([[1, 0.5, 0], [0, 2, 0.5], [0, 0.5, 1]])
    bs = cv2.filter2D(bs, -1, kernel_blur)
    omrsynth.util.general.clip_image(bs)
    bs = cv2.GaussianBlur(bs, (3, 3), 0)

    mixed = omrsynth.util.blendops.darken_only(bs, value)
    mixed = omrsynth.util.general.linear_transform(mixed, y_min=0, y_max=1, x_min=0, x_max=0.55)
    value = omrsynth.util.blendops.screen(mixed, value)
    ...
    value_copy = omrsynth.util.general.linear_transform(value_copy, y_min=0, y_max=y_max_randomized, x_min=x_min_randomized, x_max=1)
    saturation = omrsynth.util.blendops.difference(value_copy, mask)
...
def basic_texture(shape, tint='random', seed=0, scale=1.0, bright_scale=1.0, random_scale=0.2):
    ...
    hf_random_scale = omrsynth.util.general.clip_number(hf_random_scale, min_value=0.05, max_value=0.4)
    ...
    texture = omrsynth.util.blendops.burn(low_f_noise, texture, alpha=0.6 * bright_scale)

    # Then again low frequency noise to darken in a different way
    y_min = 0.45 + 0.1 * _random_add(random_scale)
    low_f_noise = _blurred_noise(shape, kernel_size, y_min=y_min)
    texture = omrsynth.util.blendops.multiply(low_f_noise, texture, alpha=0.12 * bright_scale)
    ...
    texture = omrsynth.util.blendops.addition(med_f_noise, texture, alpha=0.85 * bright_scale)
    omrsynth.util.general.clip_image(texture)
